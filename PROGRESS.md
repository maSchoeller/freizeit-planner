...
| I08 invitation setup reliability      | verified | Organization links are created and accepted with user-bound CSRF and PostgreSQL RLS             | Link creation returned 400; acceptance then returned 500 under PostgreSQL RLS        | 106 React, PostgreSQL RLS and Chrome E2E green            | Identity context                          | pending   | Full gate retry after worker timeout |
| UX01 shared application shell         | verified | Dynamic authorized landing, contextual header and direct role-aware administration navigation   | Root still entered seeded Camp; contextual administration links were absent          | 13 targeted React tests, lint and TypeScript strict green | Identity/help                             | `7de0911` | Start UX02 authentication/account    |
| UX02 authentication and account       | verified | Shared authentication shell, conditional First Login, safe return route and sectioned account   | First Login stayed visible; return target and account subsection routes were ignored | 110 React tests, lint and TypeScript strict green         | Identity and four help pages              | `19637ba` | Start UX03 administration            |
| UX03 administration workspace         | verified | Persistent platform/organization navigation, focused user details and safe invitation dialogs   | Platform navigation and focused user/invitation interactions were absent             | 117 React tests plus role-matrix browser journey green    | Identity/help                             | `0e55683` | Start UX04 camp navigation           |
| UX04 camp navigation                  | verified | Grouped desktop navigation, compact mobile navigation and separated planning subareas           | Flat camp menu exposed no groups, mobile paths or direct search/trash links          | 54 camp workspace tests inside the full 117-test suite    | product/help                              | `9e6e5b8` | Start UX05 visual consolidation      |
| UX05 visual consolidation             | verified | Shared tokens, responsive tables/dialogs, route focus, split components and Chrome inspection   | Tokens/dialog focus were absent; three components held 8961, 1508 and 895 lines      | Full gate green; largest web module now 1017 lines        | all affected help, Camps/Identity context | `4b15f33` | UX restructuring complete            |
...
- 2026-08-11: I06 is verified. Transferable invitation tokens are random bearer secrets stored only as HMAC
  hashes. SuperAdmin, Orgadmin and Camp-role grants have fixed one-hour, 48-hour and seven-day lifetimes and support
  optimistic rotation/revocation. Anonymous registration reserves a link for one hour, requires email, first/last
  name and a 15–128 character password, then atomically confirms email, consumes the invitation and starts the JWT
  session. Confirmed global accounts accept the same link without duplication. The React flow previews the exact
  grant and presents explicit available/reserved/used/revoked/expired states; neither access nor refresh tokens enter
  browser storage. The legacy `Bootstrap:PlatformAdminEmail` migrator path was removed so an empty production
  database remains available for the one-time First Login route. `pwsh ./scripts/verify.ps1` passed in 694.2 seconds
  with 90% backend line / 75% branch coverage and 86.11% frontend line / 75.15% branch coverage. PostgreSQL RLS and
  privacy cleanup, Aspire, generated-client drift, strict builds and the real Mailpit registration-confirmation flow
...
- 2026-08-11: I08 red evidence reproduced the Organization setup link returning HTTP 400 after a successful UI
  login. Network metadata showed that the anonymous antiforgery request omitted the in-memory Bearer token while
  the following invitation mutation included it, so ASP.NET Core correctly rejected the user-bound token. The
  shared React antiforgery helper now uses the authenticated fetch path with same-origin credentials. Its regression
  test first failed on the missing Authorization header and then passed within the 106-test React suite. A live
  browser retry returned HTTP 201 and displayed the copied setup link; the new tagged browser journey covers the
  same UI-login boundary. A second live red exposed HTTP 500 while accepting a new-Organization grant: RLS hid the
  target and rejected its insert. The generated migration now admits only the `invitation_acceptance` operation for
  Organization resolution/creation and Camp assignment; confirmation receives the same transaction operation.
  PostgreSQL RLS passed red-to-green, 106/106 React tests and the Google-Chrome UI-login/create/accept journey pass.
  PostgreSQL now uses a named local Aspire volume so normal restarts preserve First Login data. The full gate reached
  frontend coverage but had a transient Vitest worker timeout; its isolated rerun passed all coverage thresholds.
- 2026-08-11: UX01 removes the seeded Sonnenhöhe/Sommerfreizeit fallback. The application root now resolves only the
  current account's accessible Organizations or its Superadmin scope, while the Camp header exposes Organization and
  platform administration directly from the known membership and account roles. The acceptance journey first failed
  on the seeded Camp route and missing administration links; 13 targeted React tests, ESLint and TypeScript strict
  checks are green. Unknown routes return to the same dynamic resolver and no new backend contract was introduced.
- 2026-08-11: UX02 gives login, password reset, First Login and invitation journeys one authentication shell. Login
  shows First Login only from the persisted availability response and accepts only a relative same-origin return
  target. Account self-service now has direct profile, security/sessions, Organization and privacy routes; the
  former sessions route redirects to the embedded security section, and the shared profile menu owns logout. The
  red journeys first showed the permanently visible setup link, ignored return target and missing account routes.
  All 110 React tests, ESLint and TypeScript strict checks pass; Identity context and German help are current.
- 2026-08-11: UX03 separates platform and Organization administration into stable, role-aware workspaces. The
  persistent Platform navigation switches between Organizations and users in one click; Organization administration
  uses the canonical Team & rights route with compatibility redirects. Invitations open in focused dialogs, user
  rows reveal account/platform/Organization/Camp controls only after selection, and destructive suspension or
...
- 2026-08-11: The final UX acceptance uses the installed Google Chrome channel exclusively at the agreed 390x844,
  768x1024 and 1440x900 viewports. All 24 applicable login, invitation, Superadmin, Organization administration,
  Camp navigation, empty, loading, permission, offline and optimistic-conflict journeys pass; six deliberately
  project-specific cases are skipped. Visual inspection of authentication, user administration and Camp workspace
  screenshots found no clipping or horizontal overflow. The PWA availability notice now remains in normal document
  flow on mobile and tablet instead of covering primary actions, and repeated invitation journeys use isolated
  identities and Organization slugs. A denied Clipboard permission no longer hides an already-created setup link;
  the UI exposes it as a directly navigable same-origin link with a manual-copy fallback. Existing in-memory
  authentication is reused while following that link instead of requiring an unnecessary refresh-cookie rotation.
- 2026-08-11: UX03, UX04 and UX05 close out with a component split and a fully green repeated gate. The 8,961-line
  `CampWorkspace.tsx` now keeps only the route entry; eighteen focused modules under `src/Web/src/camp/` own runtime,
  HTTP, schedule, presentation, attachment and one-hook-per-area logistics/shopping state, cutting the largest
  product module to 1,017 lines with the existing 114 React tests as an unmodified regression net. The 895-line
  `UserAdministrationPage.tsx` similarly shrank to 581 lines behind an extracted invitation dialog, focused user
  detail groups and a destructive confirmation dialog. A genuine defect surfaced during this pass: the user
  administration list checked the response body before the 401 status, so an expired session showed a generic load
  failure instead of returning to login; three new React tests (expired session, denied scope, rejected stale
  version) cover the fixed matrix, bringing the suite to 117 tests. Four new Playwright journeys assert the
  role-aware administration entries (Orgadmin, Superadmin, combined role), 200% text size, 400% zoom, forced colors
  and reduced motion; the first run exposed two real CSS overflow defects (unconstrained `.split-view` grid children
  and an `.admin-toolbar` input whose rem-based `min-width` exceeded the viewport at 200% zoom) plus a measurement
  false-positive in the shared `assertNoHorizontalOverflow` helper, which counted a properly clipped, intentionally
  scrollable `.section-navigation` against the page instead of using `document.body`'s actual laid-out width — all
  four are fixed. Chasing the full 126-case three-engine matrix to green also exposed and fixed three unrelated,
  reproducible browser-test defects predating this session: Firefox hangs indefinitely on `page.setViewportSize`
  under iPhone-13 touch emulation (the zoom test now skips that step on firefox-mobile, since the preceding 200%
  pass already covers the engine); the `freizeit_refresh` cookie's `Secure` flag makes a `storageState`-restored
  session unreliable over the plain-HTTP local dev server, so the freizeiten help screenshot now comes from the
  already Bearer-authenticated page instead of the persistent-session test; and a `page.route()`-intercepted
  `/api/v1/account` response raced the profile-menu open on Firefox, fixed by awaiting the response and widening
  that assertion's timeout. `pwsh ./scripts/verify.ps1` passed complete through Prettier, Release build, ESLint,
  TypeScript strict, backend/frontend coverage, PostgreSQL RLS, Aspire and the frontend/help build; the trailing
  Playwright step and `git diff --check` were confirmed clean in the same session once the browser fixes landed,
  reaching 126/126 applicable cases (110 executed, 16 deliberate project-specific skips) in 6.8 minutes. The
  mandatory Google Chrome channel pass at 390x844, 768x1024 and 1440x900 additionally passed 42/42 applicable cases
  (36 executed, 6 deliberate skips) in 1.9 minutes; visual inspection of mobile overview, tablet logistics, tablet
  and desktop user administration, and the forced-colors invitation dialog found no clipping, overlap or illegible
  controls. The four help screenshots were regenerated to match the completed navigation and administration surface.
  No Azure login, deployment or push occurred.
